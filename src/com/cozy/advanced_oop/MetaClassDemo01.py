...
# 定义一个 Hello 的 class，就写一个 hello.py 模块
class Hello(object):
...
#
# 正常情况下，我们都用class Xxx...来定义类，但是，type()函数也允许我们动态创建出类来，就是说，动态语言本身支持运行期动态创建类，这和静态语言有非常大的不同，
# 要在静态语言运行期创建类，必须构造源代码字符串再调用编译器，或者借助一些工具生成字节码实现，本质上都是动态编译，会非常复杂。

# 2、metaclass：元类
# 创建类：那就必须根据metaclass创建出类；就是先定义metaclass，就可以创建类，最后创建实例。
...
class ListMetaclass(type):
    def __new__(cls, name, bases, attrs):
        attrs['add'] = lambda self, value: self.append(value) # （Python中）lambda 形参列表 : 函数返回值表达式语句
...
# 普通的 list 没有 add() 方法
L2 = list()
# print(L2.add(1)) # 报错：AttributeError: 'list' object has no attribute 'add'

# ORM 框架 --> 通过metaclass修改类定义的
...
class User(Model): # User类来操作对应的数据库表User
    # 定义类的属性到列的映射
    id = IntegerField('id')
...
# 剩下的魔术方法比如save()全部由父类Model自动完成。虽然metaclass的编写会比较复杂，但ORM的使用者用起来却异常简单。

# 2、现在，我们就按上面的接口来实现该ORM。
